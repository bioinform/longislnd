            ...
            if (dis[src].available() > 0) {
                ...
                if (buffer.hp_len() == 1) {
                    ...
                    if (buffer.event().equals(EnumEvent.DELETION) && buffer.size() != 0)
                        ...
                        if(buffer.size() != 1) throw new Exception("sub with length " + buffer.size());
                        if(Kmerizer.toByteArray(buffer.kmer(),left_flank()+1+right_flank())[left_flank()]==buffer.get(0, EnumDat.BaseCall)) {
                            throw new Exception("matching base for substitution event");
                        }
                    }
                    else if (buffer.event().equals(EnumEvent.MATCH) ) {
                        if( buffer.size() != 1) throw new Exception("match with length " + buffer.size());
                        if(Kmerizer.toByteArray(buffer.kmer(),left_flank()+1+right_flank())[left_flank()]!=buffer.get(0, EnumDat.BaseCall)) {
                            throw new Exception("unmatching base for match event");
                        }
                    }
                    */
