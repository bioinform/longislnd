 ...
 */
public class CircularShotgunSequenceDrawer extends ReferenceSequenceDrawer {
  private final static Logger log = Logger.getLogger(CircularShotgunSequenceDrawer.class.getName());
  final List<Long> refCdf = new ArrayList<>();

  public CircularShotgunSequenceDrawer(final String filename) {
    super(filename);
    long numBases = 0;
  ...
  protected Fragment getSequenceImpl(int length, RandomGenerator gen) {
    ...
    int begin = gen.nextInt(refSeq.length);
    int nCount = 0;
    final byte[] sequence = new byte[length];
    if (rc) {
      for (int ss = 0, cc = refSeq.length - 1 - begin; ss < length; ++ss, --cc) {
        sequence[ss] = EnumBP.ascii_rc(refSeq[(cc % refSeq.length + refSeq.length) % refSeq.length]);
        if (EnumBP.ascii2value(sequence[ss]) == EnumBP.N.value) {
          nCount++;
        }
      }
      // shift begin/end back to forward strain's coordinate to describe locus
      begin = ((refSeq.length - begin - length) % refSeq.length + refSeq.length) % refSeq.length;
    } else {
      for (int ss = 0; ss < length; ++ss) {
        sequence[ss] = refSeq[(begin + ss) % refSeq.length];
        if (EnumBP.ascii2value(sequence[ss]) == EnumBP.N.value) {
          nCount++;
        }
      }
    }
    if (length * Heuristics.MAX_N_FRACTION_ON_READ < nCount) {
      return null;
    }
    else {
      return new Fragment(sequence, new Locus(refFrag.getLocus().getChrom(), begin, (begin + length) % refSeq.length , rc));
    }
  }
}
