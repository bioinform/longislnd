...
public class Attributes {
    public void writeTo(HObject obj) throws Exception{
        for (Map.Entry<String, Value> entry : name_value_.entrySet()) {
            ...
            sb.append(" ");
            sb.append("(");
            for (long d : entry.getValue().dims()) {
                sb.append(" " + d);
            }
            sb.append(")");
            Object raw_buffer = entry.getValue().buffer();
            sb.append(raw_buffer.getClass().getName());
            EnumType type = null;
            if (raw_buffer instanceof String[]) {
                type = EnumType.getEnum(String[].class);
                for (String b : (String[]) raw_buffer) {
                    sb.append("|-|" + b);
                }
            } else if (raw_buffer instanceof int[]) {
                type = EnumType.getEnum(int[].class);
                for (int b : (int[]) raw_buffer) {
                    sb.append("|-|" + b);
                }
            } else if (raw_buffer instanceof float[]) {
                type = EnumType.getEnum(float[].class);
                for (float b : (float[]) raw_buffer) {
                    sb.append("|-|" + b);
                }
            } else if (raw_buffer instanceof byte[]) {
                type = EnumType.getEnum(byte[].class);
                for (byte b : (byte[]) raw_buffer) {
                    sb.append("|-|" + b);
                }
            } else if (raw_buffer instanceof short[]) {
                type = EnumType.getEnum(short[].class);
                for (short b : (short[]) raw_buffer) {
                    sb.append("|-|" + b);
                }
            } else {
                throw new UnsupportedOperationException("non implemented type");
            }
            sb.append("\n");
    ...
    public <T> void add(ncsa.hdf.object.Attribute in){
        ...
        sb.append(" ");
        sb.append("(");
        for(long entry: sizes){ sb.append(" "+entry); }
        sb.append(") ");
        ...
        /*
        if(sizes.length > 1){
            String[] tmp = (String[])in.getValue();
            StringBuilder s = new StringBuilder();
            for(int ii = 0 ; ii < sizes[0] ; ++ii){
                s.append("\n");
                for(int jj = 0 ; jj < sizes[1] ; ++jj){
                    s.append(" "+tmp[ii*(int)sizes[1]+jj]);
                }
                s.append("\n");
            }
            log.info(s.toString());
    ...
    private static enum EnumType{
        ...
        public Class<?> getType(){return class_;}
        public int type(){return h5Type_;}
        public int bytes(){return h5NumBytes_;}
        public int order(){return h5Order_;}
        public int sign(){return h5Sign_;}
    ...
    private static class Value{
        ...
        Object buffer(){return buffer_; }
        long[] dims(){return dims_; }
